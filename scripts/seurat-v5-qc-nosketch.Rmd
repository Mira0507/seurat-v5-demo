...
Next we utilize `SCTransform` to normalize the counts. This is done on each
dataset independently.

The `SCTransform` function runs in two different versions:

- [v1](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-019-1874-1): Seurat v1 
...
- [v2](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-021-02584-9): Seurat v2 
...
if (length(obj.list) > 1) {
    ...
    # 31055 features across 101668 samples within 1 assay 
    # Active assay: RNA (31055 features, 0 variable features)
    #  25 layers present: counts.1, counts.2, counts.3, counts.4, counts.5, counts.6, counts.7, counts.8, counts.9, counts.10
    # , counts.11, counts.12, counts.13, counts.14, counts.15, counts.16, counts.17, counts.18, counts.19, counts.20, counts.
    # 21, counts.22, counts.23, counts.24, counts.25
...
```{r sketch, dependson='sctransform', cache=TRUE}
# ------------------------------------------------------------------------------
...
if (sketch.dataset) {
    ...
    if (length(obj.list) > 1) {
        ...
        print("You have a single dataset. Data is not sketched.")
    }
}
...
integrated on total cells normalized using `SCTransform`. With sketching, integration is
performed on a subset of representative cells (called sketched cells) across all datasets
in a low-dimensional shared space, and the integrated data is then propagated back to the full
dataset. We employ [Canonical Correlation Analysis (CCA)](https://www.cell.com/cell/fulltext/S0092-8674(19)30559-8) 
...
non-integrated version uses the `SCTransform` assays and is labeled "SCT". For
...
identified using the `SCT` slot, not the `integrated` slot.
...
print(paste0(
    ...
    ifelse(sketch.dataset, "sketched", "sctransformed"),
    " data ..."))
```

```{r integration, cache=TRUE, dependson=c('sketch', 'sctransform')}
...
if (length(obj.list) > 1) {
    ...
    if (!sketch.dataset) {
        ...
        obj.integrated <- IntegrateLayers(
            ...
            normalization.method=selected_assay,
            verbose=FALSE
            )
...
saveRDS(obj.integrated,
        file=paste0(pref, '-integrated.rds'),
        compress=FALSE)
```
...
```{r feature_plots, results='asis', cache=TRUE, fig.height=16, fig.width=12, dependson='integrated_dimred'}
